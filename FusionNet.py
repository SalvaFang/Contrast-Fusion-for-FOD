...
def feature_vis(feats): # feaats形状: [b,c,h,w]
     ...
     channel_mean = channel_mean.transpose((0, 2, 3, 1))
     ...
     channel_mean = cv2.applyColorMap(channel_mean[0,:,:,:], cv2.COLORMAP_JET)
...
class RGBD(nn.Module):
    ...
    def forward(self,x):
        ...
        # x2=self.sobelconv(x)
        # x2=self.convup(x2)
        return F.leaky_relu(x1,negative_slope=0.1)
...
class RSU6(nn.Module):#UNet06DRES(nn.Module):
    ...
    def __init__(self, in_ch=3, mid_ch=12, out_ch=3):
        ...
        self.rebnconv4 = REBNCONV(mid_ch,mid_ch,dirate=1)
        # self.rebnconv4 = Involution2d(in_channels=mid_ch, out_channels=mid_ch, groups=4)
        self.pool4 = nn.MaxPool2d(2,stride=2,ceil_mode=True)

        self.rebnconv5 = REBNCONV(mid_ch,mid_ch,dirate=1)
        # self.rebnconv5 = Involution2d(in_channels=mid_ch, out_channels=mid_ch, groups=4)
        self.rebnconv6 = REBNCONV(mid_ch,mid_ch,dirate=2)#GUM

        # self.rebnconv6 = Involution2d(in_channels=mid_ch, out_channels=mid_ch, groups=4) #GPM

        # self.esa = ESA(mid_ch, reduction=4, bias=True)
        # self.rca = RCABlock(mid_ch, mid_ch)

        upscale = 1
        window_size = 8
        height = (640 // upscale // window_size + 1) * window_size
        width = (480 // upscale // window_size + 1) * window_size
        self.TransformerIR = SwinIR(upscale=upscale, img_size=(height, width),
                       window_size=window_size, img_range=1., depths=[2, 2, 2, 2],
                       embed_dim=60, num_heads=[2, 2,2, 2], mlp_ratio=2, upsampler='212')




        self.rebnconv5d = REBNCONV(mid_ch*2,mid_ch,dirate=1)
        # self.rebnconv5d = Involution2d(in_channels=mid_ch*2, out_channels=mid_ch, groups=4)
        self.rebnconv4d = REBNCONV(mid_ch*2,mid_ch,dirate=1)
        # self.rebnconv4d = Involution2d(in_channels=mid_ch * 2, out_channels=mid_ch, groups=4)
        self.rebnconv3d = REBNCONV(mid_ch*2,mid_ch,dirate=1)
        self.rebnconv2d = REBNCONV(mid_ch*2,mid_ch,dirate=1)
        self.rebnconv1d = REBNCONV(mid_ch*2,out_ch,dirate=1)
    ...
    def forward(self,x):
        ...
        # hx61=self.esa(hx5)          #GAM
        # hx62=self.rca(hx5)
        # hx66=torch.cat((hx61, hx62), 1)
        # hx6 = self.rebnconv6(hx66)

        # hx6=self.TransformerIR(hx5)
...
class FusionNet(nn.Module):
    def __init__(self, output):
        ...
        inf_ch = [16,32,48]
        output=1
        # self.Fire = ConvBlock(1, 16)
        # self.Fireir = ConvBlock(1, 16)
        # self.a1 = ConvBlock(16, 32)
        # self.a2 = ConvBlock(16, 32)

        # self.conv1 = ConvBlock(32, 16)
        self.conv3 = nn.Conv2d(16, 1, kernel_size=1, padding=0, bias=False)
        # self.fusion_block1 = FusionBlock_res(64, 0)
        self.stage2d = RSU6(2, 32, 16)
...
###################Nosmooth MI QCANet################
        ...
        # f3 = self.Fire(x_vis_origin) #encode
        # f3=self.a1(f3)
        #
        #
        # f4 = self.Fireir(x_inf_origin) #encode
        # f4 = self.a2(f4)

###################denosing decoupling method################
        #f1_0 = self.fusion_block1(f3, f4)#RFN 的残差融合模块

        ftmp = torch.cat([x_vis_origin, x_inf_origin], 1)


        f1_0 = self.stage2d(ftmp)#我们的Unet 融合模块

##########################decode module#####################################
        # f5 = self.conv1((f3+f4)*0.5)
        # f5 = self.conv1(f1_0)
        x = self.conv3(f1_0)

        return x, f1_0 #最终输出和融合器输出
...
class Quality(nn.Module):
    ...
    def forward(self, image_vis, M, fe):


        if fe:
            f6 = self.conv2(image_vis)
